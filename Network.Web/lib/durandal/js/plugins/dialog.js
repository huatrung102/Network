 ...
 * @requires knockout
 */
define(['durandal/system', 'durandal/app', 'durandal/composition', 'durandal/activator', 'durandal/viewEngine', 'jquery', 'knockout', 'bootstrap'], function (system, app, composition, activator, viewEngine, $, ko) {
    var contexts = {},
    ...
    */
    ...
    MessageBox.defaultViewMarkup = [
        '<div data-view="plugins/messageBox" data-bind="css: getClass(), style: getStyle()">',
            ...
            '<div class="modal-body">',
                '<p style="border: none" class="message" data-bind="html: message"></p>',
            '</div>',
    ...
    dialog = {
        ...
        getContext: function (name) {
            return contexts[name || 'bootstrap'];
        },
        /**
        ...
        show: function (obj, activationData, context) {            
            var that = this;
            var dialogContext = contexts[context || 'bootstrap'];
                    ...
                    dialogActivator.activateItem(instance, activationData).then(function (success) {
                        if (success) {
                            ...
                            if (typeof activationData !== 'undefined') {
                                if (activationData.hfull) {
                                    $(theDialog.host).addClass("dialog-full-height");
                                    $(theDialog.host).parent().addClass("dialog-full-height");
                                }
                                if (activationData.wfull) {
                                    //$(theDialog.host).addClass("dialog-full-height");
                                    $(theDialog.host).parent().addClass("dialog-full-width");
                                }
                            }
                        } else {
                            dfd.resolve(false);
